...
private:
    ...
    static bool LaunchAndInject(D3dxIniUtils& ini, const std::wstring& delayStr) {
        ...
        *filePart = L'\0';
        ...
        bool extraOk = true;
        for (size_t index = 0; index < ini.inject_dlls.size(); ++index) {
            const auto& dllPath = ini.inject_dlls[index];
            printf("[Loader] Injecting extra DLL %zu/%zu: %S\n", index + 1, ini.inject_dlls.size(), dllPath.c_str());
            const bool currentOk = NtInjectDll(pi.hProcess, dllPath.c_str());
            if (!currentOk) {
                printf("[Loader] Failed to inject extra DLL: %S\n", dllPath.c_str());
            }
            extraOk = currentOk && extraOk;
        }

        ResumeThread(pi.hThread);

        const bool moduleLoaded = WaitForModuleLoaded(pi.dwProcessId, ini.module.c_str(), 30000);
        if (moduleLoaded) {
        ...
        const bool ok = moduleLoaded && extraOk;
    ...
    static bool NtInjectDll(HANDLE process, const wchar_t* dllPath) {
        ...
        NTSTATUS status = ntAlloc(process, &remote, 0, &len, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
        if (status != 0) {
            printf("[Loader] NtAllocateVirtualMemory failed: 0x%X\n", status);
            return false;
        }

        SIZE_T written = 0;
        status = ntWrite(process, remote, const_cast<wchar_t*>(dllPath), len, &written);
        if (status != 0 || written != len) {
            printf("[Loader] NtWriteVirtualMemory failed: 0x%X\n", status);
            return false;
        }

        PVOID loadLibrary = reinterpret_cast<PVOID>(GetProcAddress(GetModuleHandleW(L"kernel32.dll"), "LoadLibraryW"));
        if (!loadLibrary) {
            printf("[Loader] Failed to resolve LoadLibraryW.\n");
            return false;
        }

        HANDLE thread = nullptr;
        status = ntThread(&thread, THREAD_ALL_ACCESS, nullptr, process, loadLibrary, remote, 0, 0, 0, 0, nullptr);
        if (status != 0 || !thread) {
            printf("[Loader] NtCreateThreadEx failed: 0x%X\n", status);
            return false;
        }
        ...
        if (exitCode == 0) {
            printf("[Loader] Remote LoadLibraryW failed for %S.\n", dllPath);
            return false;
        }
